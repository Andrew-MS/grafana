...
description: Check an implemented change against its approved change contract and deterministic verify.json evidence. Use after final verification and before opening a PR. Read only the two provided files.
model: inherit
readonly: true
---
...
1. the approved change-contract path;
...
If either input is missing, malformed, or not explicitly identified, return
`VERDICT: FAIL` and stop.

## Checklist

1. **Bounds:** every changed path matches an `in_bounds` glob and none matches
   an `out_of_bounds` glob.
2. **Evidence ran:** every command named by an acceptance criterion appears in
   `final_checks`.
3. **Evidence passed:** mode is `final`, `tree_clean` is true, `head_sha` exists,
   `all_passed` is true, and no required check was skipped.
4. **Criteria coverage:** every criterion maps to a final check or is explicitly
   tagged `manual`.
5. **Assumptions:** no `must-hold` assumption conflicts with the evidence, such
   as a single-stack contract with `split_recommended: true`.
6. **Freshness:** evidence `created_at` is later than contract `approved_at`.
7. **Behavior change:** `feature` and `bugfix` contracts include a changed test
